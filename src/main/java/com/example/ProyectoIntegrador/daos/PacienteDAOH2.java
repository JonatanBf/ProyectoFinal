...
public class PacienteDAOH2 implements Dao<Paciente>{
    ...
    public void agregar(Paciente paciente) {
        try {
            ...
            st.setDate(5, Date.valueOf(paciente.fechaAlta()));
            st.execute();
        }catch (SQLException e) {
            logger.error("Metodo Agregar: "+ e.getMessage());
        }
        logger.info("Se agrego correctamente: " + paciente.apellido() +" "+paciente.nombre()+" Con Domicilio en : "+paciente.domicilio()+" DNI: "+paciente.dni()+ " FechaAlta: "+ paciente.fechaAlta());
    }
